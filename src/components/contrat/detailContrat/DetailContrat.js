...
export default class DetailContrat extends React.Component {
    ...
    componentDidMount() {
        ...
        ContratService.getContratDetail(this.props.match.params.id).then((resultat) => {
            const contrats = resultat.data;
            console.log(contrats.clauseEtArticleList[0].titreArticle)
            this.setState({contrat: contrats, clauseEtArticle: contrats.clauseEtArticleList, loaded:true});
        })
    ...
    detailContrats(props) {
        ...
        return (
            ...
            <div>
                {props.contrats.clauseEtArticleList.map((c,index) => <h1 key={index}>{c.titreArticle}</h1>)}


                <p>Crée le : {props.contrats.contratDateCreation}</p>
                <h1>{props.contrats.contratTitre}</h1>
                <p>{props.contrats.infoSocieteRaisonSociale} <br/>
                ...
                <p>{props.contrats.employerMatricule} <br/>
    ...
    render() {
        if(this.state.loaded){
            return (
                <this.detailContrats contrats={this.state.contrat} clausesEtArticles={this.state.clauseEtArticle}/>
            );
        }else{
            return (
             <Loading/>
